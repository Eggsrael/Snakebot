...
class FollowerEnv(gym.Env):
    ...
    def _get_obs(self, yaw_rate_hint=0.0):
        ...
        # FIX: safe normalization + clamp
        prev_action_norm = clamp(
            self.prev_action / (self.action_space.n - 1),
            0.0,
            1.0
        )
    ...
    def step(self, action):
        ...
        reward += 1.0 * (
            abs(prev_rel_dist - self.target_follow_dist)
            - abs(rel_dist - self.target_follow_dist)
        )
